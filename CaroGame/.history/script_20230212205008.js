...
function checkRightDiagonal(squareArr, x, y) {
  ...
  if (count2 >= MAX_WIN) {
    ...
    for (let i = 0; i < MAX_ROW + 1; i++) {
      x0++;
      y0--;
      if(squareArr[x0 - 1][y0 + 1] == null){
        continue
      }else{
        if(squareArr[x0 - 1][y0 + 1].getValue() == squareArr[x][y].getValue() && squareArr[x0][y0].getValue() == squareArr[x][y].getValue()){
    ...
    //     break;
    //   }
    //   if (x0 == MAX_ROW || y0 < 0 || squareArr[x0 - 1][y0 + 1] == null) {
    //     continue;
    //   }
    //   console.log(`[${x0}][${y0}] = ${squareArr[x0][y0].getValue()}`);

    //   if (
    //     squareArr[x0 - 1][y0 + 1].getValue() === squareArr[x][y].getValue() &&
