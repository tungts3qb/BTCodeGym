...
function resetBoard(arr) {
  ...
  for (let i = 0; i < length; i++) {
    for (let j = 0; j < length; j++) {
      if (arr[i][j] != null) {
        if (arr[i][j].getValue() === "x") {
          // document.getElementById(`${i}${j}`);
          // console.log(`arr[${i}][${j}] = ${arr[i][j].getValue()}`);
          let list = document.getElementById(`${i}${j}`);
          console.log(
            document.getElementById(`${i}${j}`).removeChild(list.children[0])
          );
        }
        if (arr[i][j].getValue() === "0") {
          ...
          console.log(
            document
              .getElementById(`${i}${j}`)
              .childNodes[0].classList.remove("square-icon-o")
          );
        }
      }
    }
