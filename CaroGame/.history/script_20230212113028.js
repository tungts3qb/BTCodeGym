...
function Click(id) {
  ...
  if (!isNaN(x) && !isNaN(y)) {
    ...
    if (checkLeftDiagonal(squareArr, x, y)) {
      ...
      // alert(`${squareArr[x][y].getValue()} chien thang`);
      // if (confirm(`${squareArr[x][y].getValue()} chien thang`)) {
      // }
...
function checkLeftDiagonal(squareArr, x, y) {
  ...
  if (count2 >= MAX_WIN) {
    ...
    console.log(squareArr[x][y].getValue());
    for (let i = 1; i <= MAX_ROW; i++) {
      console.log(squareArr[x0][y0].getValue());
      console.log(squareArr[x0++][y0++].getValue());
      // if (
      //   squareArr[x0][y0].getValue() == squareArr[x][y].getValue() &&
      //   squareArr[x0][y0].getValue() == squareArr[x0 + i][y0 + i].getValue()
      // ) {
